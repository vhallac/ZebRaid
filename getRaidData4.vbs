...
Function ScrapeSection(table, message, defRole)
    ...
    hdrText = InnermostChild(table.rows(0).cells(0)).nodeValue
    If left(hdrText, 5) = "Empty" Then
       Exit Function
    End If

    For iCell = 0 To table.rows(1).cells.Length - 1
        hdrText = InnermostChild(table.rows(1).cells(iCell)).nodeValue
        If left(hdrText, 4) = "Name" then
            iName = iCell
        ElseIf left(hdrText, 8) = "Comments" then
            iNote = iCell
        ElseIf left(hdrText, 4) = "Pri Talent" then
            iRole = iCell
        End If
    Next

    For iRow = 2 to table.rows.Length - 1
        attendName = ""
        attendNote = ""
        attendRole = ""
        If iName <> -1 then
...
Function ScrapeRaidList(PageText)
    ...
    For iTbl = 0 to tables.Length - 1
        If tables(iTbl).className = "dataOutline" Then
           iTblIdx = iTbl
           Exit For
        End If
    Next
    ...
    For iRow = 0 to 1
        Set cells = Tables(iTblIdx).Rows(iRow).Cells
        locRowStart = iRow + 1
    ...
    If locDate <> -1 And locUrl <> -1 Then
        For iRow = locRowStart to Tables(iTblIdx).Rows.Length - 1
            Set cell1 = InnermostChild(Tables(iTblIdx).Rows(iRow).Cells(locUrl))
            scraped = scraped & _
                "<option value=""" & _
                    Replace(cell1.parentNode.getAttribute("href"), "about:", "") & """>"
            Set cell2 = InnermostChild(Tables(iTblIdx).Rows(iRow).Cells(locDate))
...
raidListPage = ReadPage("http://www.zebraguild.com/wrm4/index.php?mode=view&Sort=Date&SortDescending=1", null)
...
raidPage = ReadPage("http://www.zebraguild.com/wrm4/" & raidUrl, cookies)
